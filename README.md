...
- **Voice channel TTS** (Phase 2.2.0): say "join voice" in any allowed text channel (while you're in a voice channel) and Iris joins it. Her text replies are then ALSO spoken aloud via local neural TTS — no audio data leaves the host. Three modes via `IRIS_TTS_ENGINE`:
  ...
  - **`auto`**: detects Iris's reply language (restricted to EN/DE/KO/JA via `lingua`) and routes per-language to the engine + voice that handles it best. Default mapping: EN/JA/KO → Kokoro, DE → Piper (since Kokoro has no German). Override per-language via `IRIS_TTS_VOICE_EN` / `_DE` / `_KO` / `_JA` (format: `engine:voice_slug`).
