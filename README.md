...
- **Voice channel TTS** (Phase 2.2.0): say "join voice" in any allowed text channel (while you're in a voice channel) and Iris joins it. Her text replies are then ALSO spoken aloud via local neural TTS — no audio data leaves the host. Three modes via `IRIS_TTS_ENGINE`:
  - **`piper`** (default): lightweight, ~10× realtime CPU, robotic-ish quality. EN/GB/DE/ZH voices. ~60 MB model.
  - **`kokoro`**: 82M-param neural TTS, much higher quality (closer to ElevenLabs/OpenAI), ~3× realtime CPU / ~30× realtime GPU. Multilingual: EN, JA, KO, ZH, FR, ES, IT, PT, HI. ~310 MB model. Doesn't support German.
  - **`auto`**: detects Iris's reply language per sentence (restricted to EN/KO/JA via `lingua`) and routes to a language-matching Kokoro voice. German text gets the English voice (Kokoro has no German model; engine-hopping mid-message sounds worse). Short snippets (<10 chars) default to English to avoid misrouting things like "Ah!" or "Ja!". Override per-language via `IRIS_TTS_VOICE_EN` / `_KO` / `_JA` (format: `engine:voice_slug`). Mixed-language replies synth each segment separately and concat via ffmpeg.

  Voice slugs also configurable directly via `IRIS_PIPER_VOICE` / `IRIS_KOKORO_VOICE` when engine is fixed. Idle auto-leave after 10 min via `IRIS_DISCORD_VOICE_IDLE_SEC`. Phase 2.2.1 will add voice receive + Whisper STT inside the channel for full duplex.
