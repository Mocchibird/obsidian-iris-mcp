...
- **Voice messages** (Phase 2.1): Discord voice messages (🎙️) are auto-transcribed via local `faster-whisper` STT (CPU-only) and treated as text input — no audio data leaves the host. The .ogg blob is auto-deleted after successful transcription (the transcript is the durable record); set `IRIS_DISCORD_VOICE_AUTO_DELETE=0` to keep voice files for journaling. Model + compute type configurable via `IRIS_WHISPER_MODEL` (`base` / `small` / `medium` / `large-v3`) and `IRIS_WHISPER_COMPUTE`.
- **Voice channel — full duplex** (Phase 2.2.0 + 2.2.1): say "join voice" in any allowed text channel (while you're in a voice channel) and Iris joins it.
  - **Speak**: her text replies are spoken aloud via [Microsoft Edge TTS](https://pypi.org/project/edge-tts/) (Azure Neural voices, free, no API key). Streaming playback — audio starts ~300 ms after the request, no waiting for full synth. Per-sentence language detection routes EN → `en-US-AvaNeural`, DE → `de-DE-KatjaNeural`, JA → `ja-JP-NanamiNeural`, KO → `ko-KR-SunHiNeural`; mixed-language replies synthesize each segment with its language's voice and concatenate. Override defaults via `IRIS_TTS_VOICE_EN` / `_DE` / `_JA` / `_KO`.
  - **Listen** (Phase 2.2.1): Iris also hears you. Each utterance is segmented by silence (~700 ms hang), transcribed via local Whisper, posted to the invite text channel as `🎤 *<@you>*: <text>`, and routed through the same Claude session as a typed message. The reply gets spoken back via the existing TTS path → real conversation loop. A wake-word gate (`iris`, `hey iris`, `okay iris`) keeps Iris from reacting to every random sentence — only utterances containing a wake phrase trigger a response. Tune via `IRIS_VOICE_WAKE_WORDS` (CSV, empty = always-listen), `IRIS_VOICE_SILENCE_HANG_MS`, `IRIS_VOICE_MIN_UTTERANCE_MS`, `IRIS_VOICE_LISTEN_USERS` (CSV; inherits `IRIS_DISCORD_ALLOWED_USERS` by default).
  - Idle auto-leave after 10 min via `IRIS_DISCORD_VOICE_IDLE_SEC`. (TTS text leaves the host to Microsoft for synthesis; STT stays local; the rest of Iris stays local.)
