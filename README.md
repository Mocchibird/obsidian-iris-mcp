...
- **Voice channel TTS** (Phase 2.2.0): say "join voice" in any allowed text channel (while you're in a voice channel) and Iris joins it. Her text replies are then ALSO spoken aloud via TTS. Four modes via `IRIS_TTS_ENGINE`:
  ...
  - **`auto`**: detects reply language per sentence (script-based for CJK + lingua for Latin) and routes per-language. Defaults: **EN → Kokoro `af_sarah`** (local, fast, good for English), **JA → Edge `ja-JP-NanamiNeural`** and **KO → Edge `ko-KR-SunHiNeural`** (cloud — Kokoro's JA/KO are too weak). German text falls to the English voice. Override per-language via `IRIS_TTS_VOICE_EN` / `_KO` / `_JA` (format: `engine:voice_slug`, e.g. `edge:en-US-AvaNeural`).
