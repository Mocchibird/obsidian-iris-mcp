...
- **Voice channel TTS** (Phase 2.2.0): say "join voice" in any allowed text channel (while you're in a voice channel) and Iris joins it. Her text replies are then ALSO spoken aloud via TTS. Four modes via `IRIS_TTS_ENGINE`:
  - **`piper`** (default): lightweight, ~10× realtime CPU, robotic-ish quality. EN/GB/DE/ZH voices. ~60 MB model. 100% local.
  - **`kokoro`**: 82M-param neural TTS, ~3× realtime CPU / ~30× realtime GPU. Multilingual but weak on JA/KO. 100% local.
  - **`edge`**: Microsoft Edge TTS (Azure Neural voices via the Edge browser endpoint). State-of-the-art quality, free, no API key. ~300-500 ms HTTPS round-trip per synth. **Text leaves the host** (sent to Microsoft).
  - **`auto`**: detects reply language per sentence (script-based for CJK + lingua for Latin) and routes per-language to Edge by default — **EN → Edge `en-US-AvaNeural`**, **JA → Edge `ja-JP-NanamiNeural`**, **KO → Edge `ko-KR-SunHiNeural`**. Consistent quality across languages, sidesteps Kokoro's Pascal-GPU cuDNN issues + weak JA/KO voices. German text falls to the English voice. Override per-language via `IRIS_TTS_VOICE_EN` / `_KO` / `_JA` (e.g. `IRIS_TTS_VOICE_EN=kokoro:af_sarah` to stay 100% local for English).

  Voice slugs also configurable directly via `IRIS_PIPER_VOICE` / `IRIS_KOKORO_VOICE` / `IRIS_EDGE_VOICE`. Idle auto-leave after 10 min via `IRIS_DISCORD_VOICE_IDLE_SEC`. Phase 2.2.1 will add voice receive + Whisper STT for full duplex.
