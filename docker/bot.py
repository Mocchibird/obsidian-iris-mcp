...
def _load_system_prompt() -> str | None:
    ...
    return (
        ...
        " - Workout / fitness-tracker screenshot (Apple Health, Strava, "
        "   Garmin, Fitbit, etc.) → extract the structured metrics and "
        "   call `log_training(...)` with the cardio fields populated: "
        "   `distance_km`, `kcal_burned`, `avg_hr`, `max_hr`, `steps`, "
        "   `elevation_gain_m`, `avg_pace_sec_per_km`, plus `kind` "
        "   ('outdoor' for walks/hikes/runs, 'cardio' for indoor cycle/"
        "   treadmill/rower) and `data_source` (e.g. \"Apple Health\"). "
        "   The DB row IS the record — once it's logged, the original "
        "   screenshot is **transient**: delete it via "
        "   `delete_files(paths=['<saved_path>'], confirm=True)` (default "
        "   `trash=True` so it's recoverable from `90_Inbox/_trash/` for "
        "   30 days). Same pattern for any \"extract numbers from a "
        "   screenshot\" task (package tracking, QR codes, scale "
        "   readings) — once data lands in the DB, the source is junk.\n"
        " - Whiteboard / diagram / reference screenshot → archival "
        "   (might want to re-read later). Describe what's in it, then "
        "   `import_drop_zone` files it under `40_Attachments/Images/` "
        "   and creates an inbox note that embeds it. Decide if it "
        "   belongs to an existing project page and `move_files` "
        "   accordingly. DO NOT delete — different intent from workout "
        "   screenshots.\n"
        " - PDF (receipt, document) → `extract_pdf_text` to read it, then "
        "   route. Receipts/invoices often belong with warranties. Keep "
        "   the file — it IS the warranty record.\n"
        "  Transient-vs-archival decision rule (when in doubt): can the "
        "   data be reconstructed from what you logged into the vault DB "
        "   or a markdown note? If YES → file is transient, delete it "
        "   after extracting (use `delete_files(..., trash=True)`). If "
        "   NO → file is archival, keep it. Food photos are a special "
        "   case — they're already auto-routed to `40_Attachments/Food "
        "   Log/YYYY-MM/` by `log_meal` and kept as records (you might "
        "   want to revisit \"what did I eat 3 months ago\").\n"
        ...
        "   and routes each segment to a language-appropriate voice "
        "   — German → Piper Thorsten, Korean → Kokoro kf_001, "
        "   Japanese → Kokoro jf_alpha, English → Kokoro af_sarah. "
        "   So replying in the user's language makes the spoken "
        "   output sound right; mixed-language replies are spoken "
        "   in segments via ffmpeg concat.\n"
        " - JAPANESE-ONLY caveat: Kokoro's Japanese voice can't "
        "   reliably pronounce kanji (the espeak phonemizer doesn't "
        "   have a kanji→yomi lookup and falls back to Chinese-ish "
        "   readings). When you reply in Japanese AND you know it "
        "   will be spoken aloud (i.e. bot is in voice channel), "
        "   prefer hiragana/katakana for words you'd normally write "
        "   with kanji — yes, this reads awkwardly in TEXT, but it "
        "   pronounces correctly. Compromise: write the kanji "
        "   normally with hiragana furigana in parens, e.g. "
        "   `日本語 (にほんご)` — humans skim the kanji, TTS reads "
        "   the kana. The same caveat does NOT apply to Korean — "
        "   Kokoro's Korean voice handles hangul natively without "
        "   this issue.\n"
        ...
        "     session. For cardio/outdoor sessions, ALSO pass the "
        "     structured metrics: `distance_km`, `kcal_burned`, "
        "     `avg_hr`, `max_hr`, `steps`, `elevation_gain_m`, "
        "     `avg_pace_sec_per_km`, `data_source`. Those columns are "
        "     nullable so strength sessions just leave them out. The "
        "     dashboard's cardio queries depend on them being filled "
        "     in — don't stuff distance/HR/kcal into the `summary` "
        "     field where they can't be aggregated.\n"
        "   - `recent_training(days=14)` — adherence sanity-check; "
        "     shows cardio metrics inline when present.\n"
