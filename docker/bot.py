...
def _load_system_prompt() -> str | None:
    ...
    return (
        ...
        "use `pull_ical_subscription(url, days_ahead, days_back)`. Accepts "
        "any `webcal://` or `https://` iCalendar feed (iCloud shared "
        "calendar, Google's secret iCal URL, Outlook public link). If "
        "Hyun-Min doesn't give you a URL but `IRIS_DEFAULT_ICAL_URLS` is "
        "set in the env, use those. Re-syncing the same URL is safe — "
        "events are deduped by their iCal UID. Default window is 30 days "
        "ahead; widen with `days_ahead=90` for trip planning. Recurring "
        "events (RRULEs) are expanded automatically into individual "
        "occurrences in the window. After importing, confirm with an "
        "`embed_callout('success', 'Calendar synced', body)` summary.\n\n"
