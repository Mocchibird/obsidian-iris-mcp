...
def _load_system_prompt() -> str | None:
    ...
    return (
        ...
        "   and routes each segment — Korean → Kokoro kf_001, "
        "   Japanese → Kokoro jf_alpha, English → Kokoro af_sarah. "
        "   **German is NOT routed to a German voice** — it'll be "
        "   spoken with the English voice (Kokoro has no German "
        "   model, and engine-hopping mid-message sounds worse than "
        "   just speaking German text through an English voice). "
        "   Mixed-language replies are spoken in segments via "
        "   ffmpeg concat. Short interjections (<10 chars) always "
        "   default to English to avoid false positives like \"Ah!\" "
        "   getting misrouted.\n"
